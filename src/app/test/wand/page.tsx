...
export default function WandTrackingPage() {
  ...
  useEffect(() => {
    ...
    if (calibrationState === "calibrating") {
      // 静止判定: 加速度の急激な変化を検知したらリセット
      if (calibrationPrevAccelRef.current) {
        const prev = calibrationPrevAccelRef.current;
        const diffX = Math.abs(accel.x - prev.x);
        const diffY = Math.abs(accel.y - prev.y);
        const diffZ = Math.abs(accel.z - prev.z);

        if (
          diffX > CALIBRATION_ACCEL_THRESHOLD ||
          diffY > CALIBRATION_ACCEL_THRESHOLD ||
            ...
            {gestureResult && gestureResult.type !== "unknown" && (
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/90 rounded-xl px-6 py-3 shadow-lg text-center z-20">
                <div className="text-2xl font-bold text-purple-700">
                  ...
                  {gestureResult.type === "InvV" && "🔥 逆V字を描きました！"}
                  {gestureResult.type === "Wave" && "🌊 波の軌道を描きました！"}
                </div>
