      ...
      <div className="relative p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs tracking-wider ${isConnected ? "text-magic-glow" : "text-destructive"}`}>
        ...
        <div className="flex items-center gap-8">
          ...
          >
            {/* Outer ring */}
            <div className={`absolute inset-0 rounded-full border-2 transition-colors duration-500 ${isPowerOn ? "border-magic-glow/60" : "border-gold-dim/20"}`} />
            {/* Glow */}
            {isPowerOn && (
              <div className="absolute inset-0 rounded-full bg-magic-glow/10 blur-md animate-pulse" />
            )}
            {/* Inner circle */}
            <div className={`relative flex items-center justify-center w-16 h-16 rounded-full border transition-all duration-500 ${isPowerOn ? "border-magic-glow/50 bg-magic-glow/10" : "border-gold-dim/20 bg-stone/60"} group-hover:scale-105`}>
              <Power className={`w-7 h-7 transition-colors duration-500 ${isPowerOn ? "text-magic-glow" : "text-gold-dim/50"}`} />
            </div>
          </button>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-3">
              <Wind className={`w-4 h-4 ${isPowerOn ? "text-magic-glow" : "text-gold-dim/40"}`} />
          ...
          <div className="flex flex-col gap-2 text-right shrink-0">
            <div>
              <p className="text-[10px] uppercase tracking-[0.15em] text-gold-dim/50">{'Device'}</p>
              <p className="text-xs text-foreground tracking-wider">{'Smart Plug #1'}</p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-[0.15em] text-gold-dim/50">{'Mode'}</p>
              <p className={`text-xs tracking-wider ${isPowerOn ? "text-magic-glow" : "text-gold-dim/40"}`}>
